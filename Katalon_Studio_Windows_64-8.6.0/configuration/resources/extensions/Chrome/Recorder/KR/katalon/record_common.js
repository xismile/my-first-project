...
function getShadowRootElement(action, element, currentWindow) {
    var object = {}
    treeHTMLForRecord(action, element, object, currentWindow);
    if (element.getRootNode() instanceof ShadowRoot) {
        object['shadow_root'] = getShadowRootElement(action, element.getRootNode().host, currentWindow);
    }
    return object;
}
...
function mapDOMForRecord(action, element, currentWindow) {
    ...
    treeObject = getShadowRootElement(action, element, currentWindow);
    return treeObject;
}