...
class KURecorder {
    ...
    constructor(window) {
        ...
        this.select_change = this.select_change.bind(this);
    }
    ...
    attach(version) {
        ...
        this.rec_elementInfoDivText; // xpath text to show in this.rec_elementInfoDiv    
    ...
    processOnClickTarget (selectedElement, clickType, currentURL) {
        this.checkForNavigateAction(currentURL);
        if (clickType === "right" && selectedElement.tagName === "INPUT") {
    ...
    mouse_over(event) {
        ...
        if (this.rec_hoverElement && this.rec_hoverElement != selectedElement) {
            this.rec_hoverElement.style.outline = '';
        }
        if (window.document.documentElement) {
            nowNode = window.document.documentElement.getElementsByTagName('*').length;
        }
        this.hoveringElement = selectedElement;
        var self = this;
        if (pageLoaded === true) {
            var clickable = this.findClickableElement(selectedElement);
            // KU: Start hover handling
            if (selectedElement != this.rec_hoverElement) {
                try{
                    this.rec_hoverElement = selectedElement;
            ...
            if (clickable) {
                this.nodeInsertedLocator = selectedElement;
                setTimeout(function () {
    ...
    select_change() {
        if (this.hoveringElement != undefined) {
            if (this.hoveringElement.tagName) {
                var tagName = this.hoveringElement.tagName.toLowerCase();
                if ('select' == tagName) {
                    if (!this.hoveringElement.multiple) {
                        // KS doens't handle addSelection & removeSelection yet
                        var option = this.hoveringElement.options[this.hoveringElement.selectedIndex];
                        this.processOnChangeTarget(this.hoveringElement);
                        //this.record("select", this.ku_locatorBuilders.buildAll(this.hoveringElement), this.getOptionLocator(option));
                    } else {
